<template> 
  <div class="detail-container">
    ...
    ref="logistics"></logistics-dialog>
  </div>
</template>
<script>
  ...
  export default {
    ...
    created() {
      ...
      getOrderDetail(this.id).then(response => {
        ...
        this.$refs.logistics.express(this.order.orderSn,this.order.deliverySn,this.order.deliveryCode);
      });
    },
     mounted:function(){
        if(this.$route.query.id){
              this.orderid = this.$route.query.id;
              sessionStorage.setItem('updateOrderid',this.$route.query.id);
        }
     },

     watch: {
        $route(){
           if(this.$route.path=='/oms/orderDetail' && this.$route.query.id){
                  var newid = this.$route.query.id;//当前页新id
                  var oldid = sessionStorage.getItem('updateOrderid');//旧的id
    ...
    methods: {
      ...
      handleDeleteOrder(){
        this.$confirm('是否要进行该删除操作?', '提示', {
          ...
          deleteOrder(params).then(response=>{
            ...
            this.resetData();
            sessionStorage.removeItem('updateOrderid');
            this.$router.back();
          });
        })
      },
      ...
      resetData(){
        ...
        this.order = {},
        this.receiverDialogVisible =false,
        this.receiverInfo = Object.assign({},defaultReceiverInfo),
        this.moneyDialogVisible = false,
        this.moneyInfo = {orderId:null, freightAmount:0, discountAmount:0,status:null},
        this.messageDialogVisible = false,
        this.message = {title:null, content:null},
        this.closeDialogVisible = false,
        this.closeInfo = {note:null,id:null},
        this.markOrderDialogVisible = false,
        this.markInfo = {note:null},
        this.logisticsDialogVisible = false
      },
       handleDeliveryOrder(){
        this.$router.push({path:'/oms/deliverOrderList',query:{list:JSON.stringify([this.order])}})
      },
    }
  }
</script>
