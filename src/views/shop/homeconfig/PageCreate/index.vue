<template>
<div class="page-create app-container">
...
import {add, edit} from '@/api/shop/landpages'
export default {
  name: 'PageCreate',
  data() {
    return {
      ...
      isEditor:this.$route.path=='/shop/pageEditor',
    }
  },
  computed:{
    //映射组件的状态
  ...
  methods:{   
    ...
    save(){
      ...
      const storePage=this.isEditor?edit:add
      storePage(this.$store.state.page)
      .then(res=>{        
        this.$alert('保存成功，即将跳转到列表页面！', '保存成功', {
          confirmButtonText: 'OK',
          showClose:false,
          callback: action => {
            this.$router.push({path:'/shop/homeconfig'})
          }
        })  
      })
      .catch(res=>{
