    ...
    <div class="head-container">
      ...
      <el-dialog :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="500px">
        <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
          ...
          <el-form-item label="所属门店" prop="storeId">
            <el-select v-model="form.storeId" style="width: 178px" placeholder="请先选择门店" @change="selectChanged" >
              <el-option
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column v-if="columns.visible('verifyStatus')" prop="verifyStatus" label="核销开关" >
          <template slot-scope="scope">
            <div>
              <el-tag v-if="scope.row.verifyStatus" :type="''">开启</el-tag>
...
<script>
import crudStoreStaff from '@/api/store/staff'
...
// crud交由presenter持有
const defaultCrud = CRUD({ title: '门店店员', url: 'store/staff', sort: 'id,desc', crudMethod: { ...crudStoreStaff }})
const defaultForm = { user: {uid: null,nickname: null,avatar: null}, id: null, uid: null, avatar: null, storeId: null, staffName: null, phone: null, verifyStatus: true, status: null, addTime: null, nickname: null, storeName: null }
export default {
  name: 'storeStaff',
  ...
  methods: {
    ...
    selectChanged(value){
      for (var i=0; i<this.mystores.length; i++) {
          let mystore  =this.mystores[i];
          if(mystore.id === value) {
              this.form.storeName = mystore.name;
              break;
          }
      }
    },
