...
<script>
  ...
  export default {
    ...
    mounted:function(){
      if(this.isEdit) {
        if(this.$route.query.id){
              ...
              sessionStorage.setItem('updateProductid',this.$route.query.id);
        }
      }
    },
    ...
    watch: {
        $route(){
           if(this.isEdit){
                if(this.$route.path=='/pms/updateProduct' && this.$route.query.id){
                  var newid = this.$route.query.id;//当前页新id
                  var oldid = sessionStorage.getItem('updateProductid');//旧的id
