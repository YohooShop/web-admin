...
<script>
import { handleTagging } from '@/api/wechat/fansTag'
export default {
    ...
    computed: {
         ...
         */
        tagidsInOption(){
            let userTags=this.wxUsers.map(u=>JSON.parse(u.tagidList) || [])//示例：[[1,2],[],[1,3]]
    ...
    methods:{
        ...
        dataFormSubmit(){
            ...
            let url = this.mode=='tagging'?'batchTagging':'batchUnTagging';
            handleTagging(url,data).then(res=>{
                  this.$message({
