<template>
    <div class="panel">
        ...
        <span class="panel-content">
            <span v-if="msg.msgType=='text'" v-html="JSON.parse(msg.detail).content"></span>
            <span v-else-if="msg.msgType=='event'" >
                <el-tag size="mini" type="warning" effect="plain">事件</el-tag>
                <el-tag size="mini" type="info" effect="plain">{{JSON.parse(msg.detail).event}}</el-tag>
                {{JSON.parse(msg.detail).eventKey}}
            </span>
