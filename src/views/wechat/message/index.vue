            ...
            <div class="msg-item" v-for="(msg,index) in  dataList" :key="index">
                ...
                <div class="item-content">
                    <div class="flex justify-between margin-bottom">
                        ...
                        <div class="reply-btn">
                            <div v-if="canReply(msg.createTime) && msg.inOut == 0" @click="replyHandle(msg.openid)" class="el-icon-s-promotion">回复</div>
                        </div>
                    </div>
...
import { getList } from '@/api/wechat/message'
import { listByIds } from '@/api/wechat/fans'
...
export default {
    ...
    components: {
        WxMsgReply:()=>import('./components/wx-msg-reply'),
    ...
    methods: {
        ...
        refreshUserList(msgList){
            ...
            params.append('ids', openidList);
            listByIds(params).then(res=>{
                 this.userDataList = this.userDataList.concat(res.data)
            }).catch(e=>{
