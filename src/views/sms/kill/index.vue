    ...
    <el-table v-loading="loading" :data="data" size="small" style="width: 100%;">
      ...
      <el-table-column prop="image" label="产品主图">
        <template slot-scope="scope">
          <a :href="scope.row.pic" style="color: #42b983" target="_blank"><img :src="scope.row.pic" alt="点击打开" class="el-avatar"></a>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="timeSlot" label="秒杀时间段" />
      <el-table-column prop="stopTime" label="开始时间">
        <template slot-scope="scope">
          <span>{{ formatTimeTwo(scope.row.startTimeDate/1000) }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="stopTime" label="结束时间">
        <template slot-scope="scope">
          <span>{{ formatTimeTwo(scope.row.endTimeDate/1000) }}</span>
        </template>
      </el-table-column>
...
import { del } from '@/api/shop/killProduct'
  ...
  methods: {
    ...
    handleDialogSelectionChange(val){
      ...
      handleSelectDialogConfirm(){
        ...
        this.$router.push({path: '/sms/addkill',query:{
    ...
    edit(data) {
      console.log(data);
         this.$router.push({path: '/sms/updatekill',query:{
           id:data.id}})
      }
  }
}
