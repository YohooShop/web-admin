  ...
  export default {
    ...
    data() {
      ...
      watch: {
        $route(){
           if(this.isEdit){
                if(this.$route.path=='/sms/updateCoupon' && this.$route.query.id){
                  var newid = this.$route.query.id;//当前页新id
                  var oldid = sessionStorage.getItem('updateCouponid');//旧的id
                 
                  if(oldid!=newid){//id有变化时
                     console.log("变化了" ,newid )
                      sessionStorage.setItem('updateCouponid',this.$route.query.id);
            ...
            this.$confirm('是否提交数据', '提示', {
              ...
              if(this.isEdit){
                updateCoupon(this.$route.query.id,this.coupon).then(response=>{
                  ...
                  this.resetData();
                  sessionStorage.removeItem('updateCouponid');
                  this.$router.back();
                });
              }else{
