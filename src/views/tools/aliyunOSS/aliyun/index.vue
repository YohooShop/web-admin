...
<script>
import crudAliyunOss from '@/api/tools/aliyunoss'
...
// crud交由presenter持有
const defaultCrud = CRUD({ title: '阿里云文件', url: 'api/aliyunOssContent', crudMethod: { ...crudAliyunOss }})
export default {
  ...
  methods: {
    ...
    handleBeforeRemove(file, fileList) {
      for (let i = 0; i < this.files.length; i++) {
        if (this.files[i].uid === file.uid) {
          crudAliyunOss.del([this.files[i].id]).then(res => {})
          return true
        }
      }
    },
    ...
    download(id) {
     ...
     crudAliyunOss.download(id).then(res => {
